...
while boebot.step(TIME_STEP) != -1:
    ...
    if steps_counted % (PRINT_INTERVAL / TIME_STEP) < 1:
        print("Step %4d, Time %.2fs, %s, Speed L %.2f, R %.2frad/s, Traveled %.3fm, Limit: %.3fm, D-LF %.3fm, D-LB %.3fm, D-RF %.3fm, D-RB %.3fm." 
            % (steps_counted, time_running, state, left_speed, right_speed, average_traveled_distance, 
            distance_limit, left_front_distance, left_back_distance, right_front_distance, right_back_distance))
