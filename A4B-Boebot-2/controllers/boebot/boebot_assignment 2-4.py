...
LENGTH_BOARD = 1.20 #[m], length of the board
TO_BOARD_DISTANCE = 0.22
...
while boebot.step(TIME_STEP) != -1:
    ...
    if state == "DRIVING":
        if average_traveled_distance >= distance_limit: # passed board
            ...
            # Correct distance to the board
            if right_front_distance > TO_BOARD_DISTANCE:
                left_speed = 1.05*speed
                right_speed = 0.95*speed
            else:
                left_speed = 0.95*speed
                right_speed = 1.05*speed   
                         
            # Correct the driving angle
            if theta > ANGLE:
                left_speed = 1.05*speed
                right_speed = 0.95*speed
            else:
            ...
            # Including the distance sensors improve the robot performance.    
            # As we can see the robot is constantly adapting its speed 
            # to maintain the same distance to the board, which allows it
    ...
    if steps_counted % (PRINT_INTERVAL / TIME_STEP) < 1:
        print("Step %4d, Time %.2fs, %s, Speed L %.2f, R %.2frad/s, Traveled %.3fm, Limit: %.3fm, D-LF %.3fm, D-LB %.3fm, D-RF %.3fm, D-RB %.3fm., Theta: %.3f rad" 
            % (steps_counted, time_running, state, left_speed, right_speed, average_traveled_distance, 
            distance_limit, left_front_distance, left_back_distance, right_front_distance, right_back_distance, theta))
